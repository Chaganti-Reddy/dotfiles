...
local function get_buf_dir(buf)
  ...
  if name == '' or name:match '^term://' then return vim.uv.cwd() end
...
local function npm_script(script, dir)
  ...
  if exists(pkg) then
    ...
    if j and j.scripts and j.scripts[script] then return 'npm run ' .. script end
  end
  return nil
end

local function cmake_build_cmd(dir)
  if exists(dir .. '/CMakeLists.txt') then return 'cmake -S . -B build && cmake --build build' end
  return nil
end

local function make_cmd(target, dir)
  if exists(dir .. '/Makefile') or exists(dir .. '/makefile') then return target and ('make ' .. target) or 'make' end
  return nil
end

local function cargo_cmd(sub, dir)
  local root = vim.fn.findfile('Cargo.toml', dir .. ';')
  if root ~= '' then return 'cargo ' .. (sub or 'run') end
  return nil
end
...
local runners = {
  python = {
    run = function(path) return 'python3 ' .. vim.fn.shellescape(path) end,
  },
  javascript = {
    run = function(path, dir) return npm_script('start', dir) or ('node ' .. vim.fn.shellescape(path)) end,
  },
  typescript = {
  ...
  lua = {
    run = function(path) return 'lua ' .. vim.fn.shellescape(path) end,
  },
  c = {
    run = function(path, dir)
...
local function run_in_term(cmd, dir)
  ...
  if runner_terminal then runner_terminal:shutdown() end
...
return {
  ...
  config = function()
    ...
    map({ 'n', 't' }, '<A-r>', function()
      ...
      if vim.bo[current_buf].buftype ~= 'terminal' then last_source_buf = current_buf end
      ...
      vim.api.nvim_buf_call(source_buf, function() vim.cmd 'silent! w' end)
