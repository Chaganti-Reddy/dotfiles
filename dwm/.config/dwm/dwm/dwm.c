...
applyrules(Client *c)
{
	...
	for (i = 0; i < LENGTH(rules); i++) {
		    ...
		    (!r->instance || strstr(instance, r->instance)))
		{
			...
			if (r->isfloating) {
				c->isfloating = 1;
				if (r->floatx != -1) c->x = r->floatx;
				if (r->floaty != -1) c->y = r->floaty;
				if (r->floatw != -1) c->w = r->floatw;
				if (r->floath != -1) c->h = r->floath;
				if (r->floatborderpx != -1) c->floatborderpx = r->floatborderpx;
			}

			// Center scratchpads with no pos
	...
	updatenumlockmask();
	{
		...
		for (k = start; k <= end; k++) {
			for (i = 0; i < LENGTH(keys); i++) {
				KeySym target = (keys[i].chain != -1) ? keys[i].chain : keys[i].keysym;

				// Match if the KeySym matches this keycode's mapping
				if (target == syms[(k - start) * skip]) {
					for (j = 0; j < LENGTH(modifiers); j++) {
