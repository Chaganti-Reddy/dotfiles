...
const char *
fmt_human_net(uintmax_t num, int base)
{
	double scaled;
	size_t i;
	const char *prefix[] = { "", "Ki", "Mi", "Gi", "Ti", "Pi", "Ei", "Zi", "Yi" };
	size_t prefixlen = sizeof(prefix) / sizeof(*prefix);

	scaled = num;
	for (i = 0; i < prefixlen && scaled >= base; i++)
		scaled /= base;

	return bprintf("%.1f %sB/s", scaled, prefix[i]);
}
