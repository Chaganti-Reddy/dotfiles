...
setup_git_info() {
    ...
    if [[ "$current_user" == "karna" ]]; then
        ...
        5) editor="" ;;
        *)
            warning "Invalid choice. Defaulting to Vim."
            editor="vim"
            ;;
        esac
    fi
...
install_browser() {
    ...
    if ! command -v dialog &>/dev/null; then
        ...
        sudo pacman -S --noconfirm dialog || {
            error "Failed to install dialog. Exiting."
            exit 1
        }
    fi
...
install_multiple_packages() {
    ...
    for package in "${packages[@]}"; do
        install_package "$package" "$package" "paru -S --noconfirm --needed" || {
            echo -e "${RED}Installation of $package failed${RESET}"
            exit 1
        }
    done
}
...
install_extra_tools() {
    ...
    for app in "${tools[@]}"; do
        ...
        9)
            ...
            ;;
        10)
            install_package "youtube-music" "Youtube Music" "paru -S --noconfirm --needed"
            ;;
        *)
            error "Invalid choice: $app"
            ;;
